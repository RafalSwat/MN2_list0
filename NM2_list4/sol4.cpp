...
void exercise1()
{
	...
	/// wyliczanie wartosci z warunkow brzegowych
	for (size_t i = 0; i < 50; i++)
	{
		u[i][0] = std::pow(1 - x, 2);
		x += h;
	}
	/// wyliczanie następnych elementów
	for (size_t i = 1; i < 49; i++)
	{
		for (size_t j = 1; j < 99; j++)
		{
			/// algorytm oparty na Kincaid
			u[i][j] = (2 * std::pow(s, 2) + s) * u[i + 1][j - 1] + (1 - 4 * std::pow(s, 2) * u[i][j - 1] + (2 * std::pow(s, 2) - s) * u[i - 1][j - 1]);
			/// algorytm z oparty na wiki
